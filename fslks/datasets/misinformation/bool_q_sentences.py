...
_CSV_PATH = '/data/LHC_kitchensink/data/test/train.jsonl '
_BIOCONTRADICTION_DOWNLOAD_INSTRUCTIONS = """Temp until upload"""


class BoolQSentences(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        path = os.path.join(dl_manager.manual_dir, 'bool_q')
        # assert False, path
        ...
        tfds.core.SplitGenerator(
                ...
                gen_kwargs={"path": os.path.join(path,  "boolq_sentences_train.json")},
            ),
        tfds.core.SplitGenerator(
                name=tfds.Split.TEST,
                gen_kwargs={"path": os.path.join(path,  "boolq_sentences_dev.json")},
            ),
        ]
    ...
    def _generate_examples(self, path=_CSV_PATH):
        ...
        with tf.io.gfile.GFile(path) as f:
            for line in open(path,'r'):
                exs = json.loads(line)
                for i, row in enumerate(exs):
                    yield str(i + 1), {
                        _QUESTION: row['question'],
                        _REVIEW_TITLE: row['title'],
                        _ABSTRACT_TEXT: row['passage'],
                        _LABEL: 0 if row['answer']==True else 1 if row['answer']==False else -1 
                    }
